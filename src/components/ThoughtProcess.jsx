        ...
        {steps.map((step, index) => (
          <div key={index} className="relative flex flex-col items-center">
            <motion.div
              ...
              className="w-12 h-12 bg-black text-white flex items-center justify-center rounded-full border outline-2 outline-black outline-offset-1 z-10 transition-all hover:shadow-lg hover:shadow-gray-500"
            >
              {index + 1}
            </motion.div>
            <p className="text-sm mt-2 text-black font-medium">{step}</p>
            {index < steps.length - 1 && (
              <div className="w-10 sm:w-20 h-[2px] bg-black absolute top-6 left-12 z-0" />
            )}
          </div>
        ))}
      </div>
